...
function create (collection, object, callback) {
  logger.debug('db-store: create', collection, JSON.stringify(object));
  if (typeof object === 'undefined') {
    logger.debug('db-store: create ERROR', 'wrong data object cannot be empty and object.id.length > 0');
  }
  ...
  $fh.db({
    ...
    if (err) {
      ...
      var items = _.map(data.list, function(itemEntry){
        logger.debug("itemEntry: " + JSON.stringify(itemEntry));
...
function update (collection, object, callback) {
  logger.debug('db-store: update', collection, JSON.stringify(object));
  if (!object) {
  ...
  setTimeout(function () {
    var options = {
     ...
     $fh.db(options, function(err, data){
       if (err){
         logger.debug("db-store: update (list) ERROR ", err);
         callback(err, null);
       } else {
         logger.debug('db-store: update data.count', data.count);
         if (data.count <= 0) {
           // Create
...
function remove (collection, object) {
  logger.info('db-store: remove', collection, JSON.stringify(object));
}

function list (collection, filter, callback) {
  logger.info('db-store: list', collection, filter);
  var _filter = {
    act: "list",
    type: collection
  };
  ...
  $fh.db(_filter, function(err, data){
    logger.info('_filter', _filter, 'err', err, 'data', data);
    if (err) {
      callback(err, null);
    } else {
      var items = _.map(data.list, function(itemEntry){
        logger.info("itemEntry: " + JSON.stringify(itemEntry));
        return itemEntry.fields;
      });
...
function updateOld (customerid, device) {
  logger.debug('last-time for ', customerid, device);
    ...
    var options = {
     ...
     $fh.db(options, function(err, data){
       if (err){
         ...
         if (data.count <= 0) {
           logger.debug("updateLastTime->customerid: ", customerid, 'not found');
         } else {
